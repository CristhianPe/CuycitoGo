...
function saveLocal() {
    try {
        if (typeof appState !== 'undefined' && appState) {
            if (appState.catalog) localStorage.setItem("cuycito_catalog", JSON.stringify(appState.catalog));
            if (appState.subscriptions) localStorage.setItem("cuycito_subscriptions", JSON.stringify(appState.subscriptions));
            if (appState.masterAccounts) localStorage.setItem("cuycito_masterAccounts", JSON.stringify(appState.masterAccounts));
            if (appState.clients) localStorage.setItem("cuycito_clients", JSON.stringify(appState.clients));
            if (appState.services) localStorage.setItem("cuycito_services", JSON.stringify(appState.services));
            if (appState.history) localStorage.setItem("cuycito_history", JSON.stringify(appState.history));
            if (appState.postits) localStorage.setItem("cuycito_postits", JSON.stringify(appState.postits));
        }
    } catch(e) {
        console.warn("saveLocal warning:", e);
    }
}
...
window.compressImageFileToDataUrl = compressImageFileToDataUrl;
window.readFileAsDataURL = compressImageFileToDataUrl;
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        try {
            ...
            subSnap.forEach(d => {
                const data = { ...d.data(), id: d.id, _docId: d.id };
                const pNorm = (data.person || '').trim().toLowerCase();
                const sNorm = (data.service || '').trim().toLowerCase();
                const startNorm = (data.startDate || '').trim();
                const endNorm = (data.endDate || '').trim();
                const pinNorm = (data.pin || '').trim().toLowerCase();
                ...
                if (pNorm.includes('sin asignar') || pNorm.startsWith('sin ') || pNorm === '' || sNorm.includes('disney premium 7') || sNorm.includes('777')) {
                    subsToDelete.push(d.id);
                    return;
                }

                const signature = `${pNorm}|${sNorm}|${startNorm}|${endNorm}|${pinNorm}`;
                if (!data.isDemo && signature && seenSubSignatures.has(signature)) {
                    subsToDelete.push(d.id);
                    return;
                }

                if (!data.isDemo && signature) {
                    seenSubSignatures.add(signature);
                }

                appState.subscriptions.push(data);
            });

            // Eliminar duplicados detectados en Firestore
            subsToDelete.forEach(async (idToDel) => {
                try { await deleteDoc(doc(db, "subscriptions", idToDel)); } catch(e){}
            });
            ...
            masterSnap.forEach(d => {
                const mData = { ...d.data(), id: d.id, _docId: d.id };
                
                // Sanear cupos duplicados en la cuenta matriz
                if (Array.isArray(mData.profiles)) {
                    let hasDupSlot = false;
                    const seenSlots = new Set();
                    mData.profiles = mData.profiles.map(p => {
                    ...
                    if (hasDupSlot) {
                        setDoc(doc(db, "masterAccounts", d.id), { profiles: mData.profiles }, { merge: true }).catch(()=>{});
                    }
                }

                appState.masterAccounts.push(mData);
            });
            ...
            catalogSnap.forEach(d => {
                ...
                remoteCatalog.push(item);
            });

            let localCatalog = [];
            try {
                const saved = localStorage.getItem("cuycito_catalog");
                if (saved) localCatalog = JSON.parse(saved);
            } catch(e){}

            const mergedCatalog = [...remoteCatalog];
            if (Array.isArray(localCatalog)) {
                localCatalog.forEach(loc => {
                    if (!mergedCatalog.some(m => m.id === loc.id)) {
                        mergedCatalog.push(loc);
                    }
                });
            }

            appState.catalog = mergedCatalog.length > 0 ? mergedCatalog : [...DEFAULT_CATALOG_ITEMS];
            saveLocal();
            ...
            const allServices = window.getAllUniqueServices(cloudServices);
...
window.switchTab = (tabId) => {
    ...
    ['subs', 'master', 'finance', 'clients', 'catalog', 'recharges', 'games', 'news', 'cartelera', 'ai-agent', 'ai-pricing'].forEach(id => {
        ...
        if(view) {
            ...
            if(id === cleanId) {
                ...
                if(id==='master' || id==='finance' || id==='clients' || id==='catalog' || id==='recharges' || id==='games' || id==='news' || id==='cartelera' || id==='ai-agent' || id==='ai-pricing') {
        ...
        if(btn) {
            ...
            if (id === 'ai-pricing') activeColorClass = "text-emerald-400 border-b-2 border-emerald-400";
    ...
    if(cleanId === 'ai-pricing' && typeof window.renderPricingTable === 'function') {
...
window.getAllUniqueServices = (additionalServices = []) => {
    const rawServices = [
        ...DEFAULT_SERVICES,
        ...(appState.services || []),
        ...(additionalServices || []),
        ...(appState.masterAccounts || []).map(m => m.service),
        ...(appState.subscriptions || []).map(s => s.service),
        ...(appState.catalog || []).map(c => c.linkedService || (c.isCombo ? null : c.title))
    ];
    ...
    rawServices.forEach(s => {
        ...
        if (!trimmed || trimmed === '__NEW__' || trimmed.startsWith('Combo:') || trimmed.startsWith('Combo ') || trimmed.startsWith('Mega Combo')) return;
...
window.updateAllServiceDropdowns = () => {
    const serviceList = window.getAllUniqueServices();
    appState.services = serviceList;
...
window.openCreateMasterModal = () => {
    ...
    if (document.getElementById('mProvider')) document.getElementById('mProvider').value = '';
    if (document.getElementById('mProviderPhone')) document.getElementById('mProviderPhone').value = '';
    if (document.getElementById('mCapacity')) document.getElementById('mCapacity').value = '5';
    if (document.getElementById('mCost')) document.getElementById('mCost').value = '';
    if (document.getElementById('mCurrency')) document.getElementById('mCurrency').value = 'PEN';
    
    const todayFormatted = window.formatDateDDMMYYYY(new Date());
    window.setDatePickerValue('mStartDate', todayFormatted);
    if (document.getElementById('mMonths')) document.getElementById('mMonths').value = '1';
    if (document.getElementById('mEmail')) document.getElementById('mEmail').value = '';
    if (document.getElementById('mPass')) document.getElementById('mPass').value = '';
    if (document.getElementById('mHidePass')) document.getElementById('mHidePass').checked = false;
...
window.saveMasterAccount = async () => {
    ...
    const providerPhone = (document.getElementById('mProviderPhone')?.value || '').trim();
    ...
    const newTx = { 
        ...
        date: startDate, 
        type: 'COMPRA', 
        person: provider || 'Proveedor', 
        service: `Compra Matriz ${service} [${masterCode}] (${email})`, 
        amount: cost, 
        currency,
        masterAccountId: id
    };
...
window.cleanDuplicateTransactions = async (showAlert = false) => {
    let purgedCount = 0;
    let fixedSlotsCount = 0;

    // 1. Saneamiento de Suscripciones / Operaciones Duplicadas
    const seenSignatures = new Set();
    const cleanSubscriptions = [];

    for (const sub of (appState.subscriptions || [])) {
        const pNorm = (sub.person || '').trim().toLowerCase();
        const sNorm = (sub.service || '').trim().toLowerCase();
        const startNorm = (sub.startDate || '').trim();
        const endNorm = (sub.endDate || '').trim();
        const pinNorm = (sub.pin || '').trim().toLowerCase();

        // Eliminar registros fantasmas o inválidos
        if (pNorm.includes('sin asignar') || pNorm.startsWith('sin ') || pNorm === '' || sNorm.includes('disney premium 7') || sNorm.includes('777')) {
            if (sub.id) {
                try { await deleteDoc(doc(db, "subscriptions", sub.id)); } catch(e){}
                purgedCount++;
            }
            continue;
        }

        // Si es demo, conservarla
        if (sub.isDemo) {
            cleanSubscriptions.push(sub);
            continue;
        }

        // Clave unívoca de duplicidad estricta
        const signature = `${pNorm}|${sNorm}|${startNorm}|${endNorm}|${pinNorm}`;
    ...
    // 2. Saneamiento de Cupos/Slots Duplicados en Cuentas Matriz
    for (const acc of (appState.masterAccounts || [])) {
        if (Array.isArray(acc.profiles)) {
            let hasDuplicateSlots = false;
            const seenAccountSubs = new Set();
            const cleanProfiles = acc.profiles.map(p => {
                if (p && typeof p === 'string') {
                    if (seenAccountSubs.has(p)) {
                        hasDuplicateSlots = true;
                        fixedSlotsCount++;
                        return null;
                    }
                    seenAccountSubs.add(p);
                }
                return p;
            });

            if (hasDuplicateSlots) {
                acc.profiles = cleanProfiles;
                try {
                    await setDoc(doc(db, "masterAccounts", acc.id), { profiles: cleanProfiles }, { merge: true });
                } catch(e){}
            }
        }
    }
    ...
    if (showAlert) {
        if (purgedCount > 0 || fixedSlotsCount > 0) {
            alert(`🧹 ¡Limpieza de Base de Datos completada!\n\n- ${purgedCount} transacciones duplicadas o corruptas purgadas.\n- ${fixedSlotsCount} cupos duplicados corregidos en Cuentas Raíz.`);
        } else {
            alert(`✅ La base de datos está limpia y sin registros duplicados.`);
        }
    }
};
...
let masterActiveFilter = 'ALL';
if (!window.expandedMasterSlots) window.expandedMasterSlots = new Set();

window.filterMasterAccounts = (filterType) => {
    masterActiveFilter = filterType || 'ALL';
    ['ALL', 'ACTIVE', 'EXPIRED'].forEach(type => {
        const btn = document.getElementById(`btnMasterFilter_${type}`);
        if (btn) {
            if (type === masterActiveFilter) {
                btn.className = 'bg-cuycito-gold text-black text-xs font-black px-4 py-2 rounded-xl transition shadow glow-gold whitespace-nowrap cursor-pointer';
            } else if (type === 'ACTIVE') {
                btn.className = 'bg-[#141414] hover:bg-gray-800 border border-gray-800 text-emerald-400 hover:text-emerald-300 text-xs font-bold px-4 py-2 rounded-xl transition whitespace-nowrap cursor-pointer';
            } else if (type === 'EXPIRED') {
                btn.className = 'bg-[#141414] hover:bg-gray-800 border border-gray-800 text-red-400 hover:text-red-300 text-xs font-bold px-4 py-2 rounded-xl transition whitespace-nowrap cursor-pointer';
            } else {
                btn.className = 'bg-[#141414] hover:bg-gray-800 border border-gray-800 text-gray-300 hover:text-white text-xs font-bold px-4 py-2 rounded-xl transition whitespace-nowrap cursor-pointer';
            }
        }
    });
    window.renderMasterAccounts();
...
window.toggleMasterSlots = (accId) => {
    if (window.expandedMasterSlots.has(accId)) {
        window.expandedMasterSlots.delete(accId);
    } else {
        window.expandedMasterSlots.add(accId);
    }
    window.renderMasterAccounts();
};

window.openRenewMasterModal = (accId) => {
    const acc = (appState.masterAccounts || []).find(a => a.id === accId);
    if (!acc) return;
    const masterCode = acc.masterCode || window.getMasterCode(acc);
    
    if (document.getElementById('renewMasterId')) document.getElementById('renewMasterId').value = acc.id;
    if (document.getElementById('renewMasterLabel')) document.getElementById('renewMasterLabel').innerText = `${acc.service || 'Servicio'} [${masterCode}] (${acc.email || ''})`;
    if (document.getElementById('renewMasterCurrentEndDate')) document.getElementById('renewMasterCurrentEndDate').innerText = window.formatDateDDMMYYYY(acc.endDate) || 'Sin fecha';
    if (document.getElementById('renewMasterProviderName')) document.getElementById('renewMasterProviderName').innerText = `${acc.provider || 'Sin proveedor'}${acc.providerPhone ? ' (' + acc.providerPhone + ')' : ''}`;
    
    if (document.getElementById('renewMasterMonths')) document.getElementById('renewMasterMonths').value = '1';
    if (document.getElementById('renewMasterCost')) document.getElementById('renewMasterCost').value = (acc.cost || 0).toString();
    if (document.getElementById('renewMasterNotes')) document.getElementById('renewMasterNotes').value = '';

    window.recalculateRenewMasterEndDate();

    const modal = document.getElementById('renewMasterModal');
    if (modal) {
        modal.classList.remove('hidden');
        window.initDatePickers();
    }
...
window.recalculateRenewMasterEndDate = () => {
    const id = document.getElementById('renewMasterId')?.value;
    const acc = (appState.masterAccounts || []).find(a => a.id === id);
    if (!acc) return;

    const months = parseInt(document.getElementById('renewMasterMonths')?.value) || 1;
    const currentEnd = window.parseDateUniversal(acc.endDate) || new Date();
    const now = new Date();

    const baseDate = currentEnd < now ? now : currentEnd;
    const newEndObj = new Date(baseDate.getTime() + (months * 30 * 86400000));
    const newEndStr = window.formatDateDDMMYYYY(newEndObj);

    if (document.getElementById('renewMasterNewEndDate')) {
        document.getElementById('renewMasterNewEndDate').value = newEndStr;
    }
};

window.saveRenewMasterAccount = async () => {
    const id = document.getElementById('renewMasterId')?.value;
    const acc = (appState.masterAccounts || []).find(a => a.id === id);
    if (!acc) return;

    const months = parseInt(document.getElementById('renewMasterMonths')?.value) || 1;
    const cost = parseFloat(document.getElementById('renewMasterCost')?.value) || 0;
    const newEndDate = (document.getElementById('renewMasterNewEndDate')?.value || '').trim();
    const notes = (document.getElementById('renewMasterNotes')?.value || '').trim();

    if (!newEndDate) {
        alert("Por favor ingresa la nueva fecha de culminación.");
        return;
    }

    const masterCode = acc.masterCode || window.getMasterCode(acc);
    if (!confirm(`¿Confirmas la RENOVACIÓN de la Cuenta Matriz "${acc.service}" [${masterCode}] por ${months} mes(es) hasta el ${newEndDate} por un costo de ${acc.currency === 'USD' ? '$' : 'S/'} ${cost.toFixed(2)}?`)) {
        return;
    }

    const prevEndDate = acc.endDate;
    acc.endDate = newEndDate;
    acc.cost = cost;

    const txId = 'tx_mas_ren_' + Date.now();
    const todayStr = window.formatDateDDMMYYYY(new Date());
    const newTx = {
        id: txId,
        date: todayStr,
        type: 'RENOVACION_MATRIZ',
        person: acc.provider || 'Proveedor',
        service: `Renovación Matriz ${acc.service} [${masterCode}] (${acc.email}) hasta ${newEndDate}${notes ? ' - ' + notes : ''}`,
        amount: cost,
    ...
    document.getElementById('renewMasterModal')?.classList.add('hidden');
    saveLocal();
    window.renderMasterAccounts();
    window.renderFinance();
    alert(`🔄 ¡Cuenta Matriz "${acc.service}" [${masterCode}] renovada con éxito hasta el ${newEndDate}!`);
    ...
    try {
        ...
        console.error("Error guardando renovación en Firestore:", e);
    }
};

window.calculateEndDateFromStart = (startDateStr, months = 1) => {
    if (!startDateStr) return '';
    const parsed = window.parseDateUniversal ? window.parseDateUniversal(startDateStr) : new Date(startDateStr);
    if (!parsed || isNaN(parsed.getTime())) return '';
    const endObj = new Date(parsed.getTime());
    endObj.setMonth(endObj.getMonth() + months);
    return window.formatDateDDMMYYYY ? window.formatDateDDMMYYYY(endObj) : endObj.toISOString().split('T')[0];
};

window.setMasterStartDateToday = (mode = 'edit') => {
    const todayStr = window.formatDateDDMMYYYY ? window.formatDateDDMMYYYY(new Date()) : new Date().toISOString().split('T')[0];
    if (mode === 'edit') {
        const input = document.getElementById('editMasterStartDate');
        if (input) input.value = todayStr;
        window.recalculateEditMasterEndDate();
    } else {
        const input = document.getElementById('mStartDate');
        if (input) input.value = todayStr;
        window.recalculateCreateMasterEndDate();
    }
};

window.recalculateEditMasterEndDate = () => {
    const startStr = document.getElementById('editMasterStartDate')?.value;
    if (!startStr) return;
    const newEnd = window.calculateEndDateFromStart(startStr, 1);
    if (newEnd && document.getElementById('editMasterEndDate')) {
        document.getElementById('editMasterEndDate').value = newEnd;
    }
};

window.recalculateCreateMasterEndDate = () => {
    const startStr = document.getElementById('mStartDate')?.value;
    const months = parseInt(document.getElementById('mMonths')?.value) || 1;
    if (!startStr) return;
    const newEnd = window.calculateEndDateFromStart(startStr, months);
    if (newEnd && document.getElementById('mEndDate')) {
        document.getElementById('mEndDate').value = newEnd;
    }
};
...
window.openEditMasterModal = (accId) => {
    const acc = (appState.masterAccounts || []).find(a => a.id === accId);
    if(!acc) return;
    const todayStr = window.formatDateDDMMYYYY ? window.formatDateDDMMYYYY(new Date()) : '';
    if (document.getElementById('editMasterId')) document.getElementById('editMasterId').value = acc.id;
    if (document.getElementById('editMasterService')) document.getElementById('editMasterService').value = acc.service || '';
    if (document.getElementById('editMasterEmail')) document.getElementById('editMasterEmail').value = acc.email || '';
    if (document.getElementById('editMasterPass')) document.getElementById('editMasterPass').value = acc.pass || '';
    if (document.getElementById('editMasterProvider')) document.getElementById('editMasterProvider').value = acc.provider || '';
    if (document.getElementById('editMasterProviderPhone')) document.getElementById('editMasterProviderPhone').value = acc.providerPhone || '';
    if (document.getElementById('editMasterCost')) document.getElementById('editMasterCost').value = (acc.cost || 0).toString();
    if (document.getElementById('editMasterCurrency')) document.getElementById('editMasterCurrency').value = acc.currency || 'PEN';
    if (document.getElementById('editMasterCapacity')) document.getElementById('editMasterCapacity').value = acc.capacity || 5;
    if (document.getElementById('editMasterStartDate')) document.getElementById('editMasterStartDate').value = acc.startDate || todayStr;
    if (document.getElementById('editMasterEndDate')) document.getElementById('editMasterEndDate').value = acc.endDate || '';
    if (document.getElementById('editMasterHidePass')) document.getElementById('editMasterHidePass').checked = !!acc.hidePasswordFromClient;

    if (!acc.endDate && (acc.startDate || todayStr)) {
        window.recalculateEditMasterEndDate();
    }
...
window.saveEditMasterModal = async () => {
    ...
    const masterCode = acc.masterCode || window.getMasterCode(acc);
    if (!confirm(`¿Confirmas la actualización de datos para la Cuenta Raíz [${masterCode}] "${acc.service}"?`)) {
        return;
    }

    const newService = (document.getElementById('editMasterService')?.value || '').trim() || acc.service;
    ...
    const newProvider = (document.getElementById('editMasterProvider')?.value || '').trim();
    const newProviderPhone = (document.getElementById('editMasterProviderPhone')?.value || '').trim();
    const newCost = parseFloat(document.getElementById('editMasterCost')?.value) || acc.cost;
    const newCurrency = document.getElementById('editMasterCurrency')?.value || acc.currency || 'PEN';
    const newCapacity = parseInt(document.getElementById('editMasterCapacity')?.value) || acc.capacity;
    const newStartDate = (document.getElementById('editMasterStartDate')?.value || '').trim() || acc.startDate;
    const newEndDate = (document.getElementById('editMasterEndDate')?.value || '').trim() || acc.endDate;
    const newHidePass = !!document.getElementById('editMasterHidePass')?.checked;
    
    const oldEndDate = acc.endDate;
    acc.service = newService;
    acc.email = newEmail;
    acc.pass = newPass;
    acc.provider = newProvider;
    acc.providerPhone = newProviderPhone;
    acc.cost = newCost;
    acc.currency = newCurrency;
    acc.startDate = newStartDate;
    acc.endDate = newEndDate;
    ...
    if (oldEndDate !== newEndDate) {
        const txId = 'tx_mas_edit_' + Date.now();
        const todayStr = window.formatDateDDMMYYYY(new Date());
        const newTx = {
            id: txId,
            date: todayStr,
            type: 'CAMBIO_FECHA_MATRIZ',
            person: newProvider || 'Proveedor',
            service: `Cambio de Fecha Culminación [${masterCode}] (${newService}): ${oldEndDate} ➔ ${newEndDate}`,
            amount: 0,
        ...
        if (!Array.isArray(appState.history)) appState.history = [];
        appState.history.push(newTx);
        setDoc(doc(db, "history", txId), newTx).catch(() => {});
    }
    ...
    window.renderMasterAccounts();
    window.renderActiveTable();
    alert(`✅ ¡Cuenta Raíz "${acc.service}" [${masterCode}] actualizada exitosamente!`);
...
window.deleteMasterAccount = async (accId) => {
    ...
    let msg = `⚠️ ¿Estás completamente seguro de ELIMINAR la Cuenta Raíz ${accName}?`;
    if (linkedSubs.length > 0) {
        msg += `\n\n🚨 ATENCIÓN: Esta cuenta tiene ${linkedSubs.length} servicio(s) asignado(s) a clientes.\nAl eliminarla, se eliminarán también estos servicios activos para evitar cuentas huérfanas en los perfiles.`;
    }
    msg += `\n\nEsta acción no se puede deshacer.`;

    if (confirm(msg)) {
        const txId = 'tx_mas_del_' + Date.now();
        const todayStr = window.formatDateDDMMYYYY(new Date());
        const delTx = {
            id: txId,
            date: todayStr,
            type: 'BAJA_MATRIZ',
            person: (acc ? acc.provider : 'Proveedor'),
            service: `Eliminación de Cuenta Matriz [${accCode}] (${acc ? acc.service : ''} - ${acc ? acc.email : ''})`,
            amount: 0,
            currency: acc ? acc.currency : 'PEN',
            masterAccountId: accId
        };
        if (!Array.isArray(appState.history)) appState.history = [];
        appState.history.push(delTx);
        setDoc(doc(db, "history", txId), delTx).catch(() => {});
        ...
        window.renderMasterAccounts();
        window.renderActiveTable();
        alert(`✅ Cuenta Raíz [${accCode}] eliminada correctamente de la base de datos.`);
    }
};
...
window.refreshCatalogOnly = async () => {
    ...
    try {
        ...
        // Combinar inteligentemente con ítems locales para no perder nada recién creado
        const currentLocal = appState.catalog || [];
        ...
        appState.catalog = merged.length > 0 ? merged : [...DEFAULT_CATALOG_ITEMS];
        saveLocal();
        window.renderCatalog();
...
window.renderComboServiceRows = () => {
    ...
    const allServices = window.getAllUniqueServices();
    let html = '';

    currentComboRows.forEach((row, index) => {
        const isCustom = row.isCustom || (row.service && !allServices.some(s => s && s.toLowerCase() === (row.service || '').toLowerCase()));
        let optionsHTML = '<option value="">-- Seleccionar Servicio --</option>';
        allServices.forEach(s => {
            if (!s) return;
            const sel = (!isCustom && (row.service || '').toLowerCase() === s.toLowerCase()) ? 'selected' : '';
            optionsHTML += `<option value="${s}" ${sel}>${s}</option>`;
        });
        optionsHTML += `<option value="__NEW__" ${isCustom ? 'selected' : ''} class="text-cuycito-gold font-bold">➕ Escribir / Agregar Nuevo Servicio...</option>`;
        ...
        <div class="bg-black/60 border border-gray-800 rounded-xl p-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 text-xs">
            <div class="flex flex-col gap-1.5 flex-1">
                <div class="flex items-center gap-2">
                    ...
                    <select onchange="window.handleComboRowSelectChange(${index}, this.value)" class="w-full bg-[#111] border border-gray-700 rounded-lg p-2 text-white font-bold outline-none focus:border-cuycito-gold">
                        ${optionsHTML}
                    </select>
                </div>
                <input type="text" id="comboRowCustom_${index}" placeholder="Escribe el nombre del servicio..." value="${isCustom ? (row.service || '') : ''}" oninput="window.updateComboRowCustomService(${index}, this.value)" class="${isCustom ? '' : 'hidden'} w-full bg-[#1a1a1a] border border-cuycito-gold rounded-lg p-1.5 text-cuycito-gold font-bold outline-none text-xs pl-8">
            </div>
            <div class="flex items-center gap-2">
                <div class="w-28">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.50" min="0" value="${row.regularPrice !== undefined ? row.regularPrice : 0}" oninput="window.updateComboRowPrice(${index}, 'regularPrice', this.value)" class="w-full bg-[#111] border border-gray-700 rounded-lg p-2 pl-6 text-gray-300 font-bold outline-none">
                    </div>
                </div>
                <div class="w-28">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.50" min="0" value="${row.comboPrice !== undefined ? row.comboPrice : 0}" oninput="window.updateComboRowPrice(${index}, 'comboPrice', this.value)" class="w-full bg-[#111] border border-emerald-500/50 rounded-lg p-2 pl-6 text-cuycito-gold font-black outline-none focus:border-emerald-400">
                    </div>
                </div>
                ${currentComboRows.length > 2 ? `
                <button type="button" onclick="window.removeComboServiceRow(${index})" class="text-gray-500 hover:text-cuycito-red p-2 rounded-lg transition mt-3 sm:mt-0 cursor-pointer" title="Eliminar servicio">
                    <i class="fa-solid fa-trash"></i>
...
window.handleComboRowSelectChange = (index, value) => {
    if (!currentComboRows[index]) return;
    if (value === '__NEW__') {
        currentComboRows[index].isCustom = true;
        currentComboRows[index].service = '';
    } else {
        currentComboRows[index].isCustom = false;
        currentComboRows[index].service = value;
    }
    window.renderComboServiceRows();
    if (value === '__NEW__') {
        setTimeout(() => {
            const input = document.getElementById(`comboRowCustom_${index}`);
            if (input) input.focus();
        }, 50);
    }
};

window.updateComboRowCustomService = (index, value) => {
...
window.recalculateComboPrices = () => {
    ...
    currentComboRows.forEach(r => {
        ...
        if (r.service && r.service.trim()) servicesNames.push(r.service.trim());
    });
    ...
    if (titleInput && (!titleInput.value.trim() || titleInput.value.startsWith('Combo ') || titleInput.value.startsWith('Mega Combo'))) {
...
window.saveComboItem = async () => {
    const saveBtn = document.getElementById('btnSaveComboItem') || document.querySelector('#comboModal button[onclick*="saveComboItem"]');
    const originalBtnHTML = '<i class="fa-solid fa-cloud-arrow-up"></i> Guardar Combo en Tienda';
    const statusLabel = document.getElementById('dbStatus');

    try {
        // 1. Recoger datos del formulario
        const id = (document.getElementById('comboId')?.value || '').trim() || ('combo_' + Date.now());
        let title = (document.getElementById('comboTitle')?.value || '').trim();
        const desc = (document.getElementById('comboDesc')?.value || '').trim() || 'Disfruta de tus plataformas favoritas en un solo combo con perfiles privados independientes, calidad 4K Ultra HD y garantía total 30 días.';
        ...
        const advantages = advantagesText ? advantagesText.split('\n').map(a => a.trim()).filter(Boolean) : [
            '✔ Perfiles Privados con PIN independiente',
            '✔ Calidad 4K Ultra HD & Audio Espacial',
            '✔ Garantía total 30 días con soporte continuo',
            '✔ Activación y entrega inmediata'
        ];

        // 2. Validar filas del combo
        if (!currentComboRows || currentComboRows.length < 2) {
            return alert("⚠️ Agrega al menos 2 servicios al combo.");
        }

        let regularTotal = 0;
        let comboTotal = 0;
        const cleanComboServices = [];
        const serviceNames = [];

        const container = document.getElementById('comboServicesContainer');
        const selects = container ? container.querySelectorAll('select') : [];

        for (let i = 0; i < currentComboRows.length; i++) {
            const row = currentComboRows[i];
            let sName = String(row.service || '').trim();
            
            const customInput = document.getElementById(`comboRowCustom_${i}`);
            if (customInput && !customInput.classList.contains('hidden') && customInput.value.trim()) {
                sName = customInput.value.trim();
            } else if (!sName && selects[i] && selects[i].value && selects[i].value !== '__NEW__') {
                sName = selects[i].value.trim();
            }

            if (!sName) {
                return alert(`⚠️ Por favor selecciona o escribe el nombre del servicio en el cupo #${i + 1}.`);
            }

            const regP = parseFloat(row.regularPrice) || 0;
            const comP = parseFloat(row.comboPrice) || 0;
            regularTotal += regP;
            comboTotal += comP;
            serviceNames.push(sName);

            cleanComboServices.push({
                service: sName,
                regularPrice: regP,
                comboPrice: comP
            });
        ...
        // Si el título estaba vacío, autogenerarlo inteligentemente
        if (!title) {
            if (serviceNames.length === 2) {
                title = `Combo Dúo: ${serviceNames.join(' + ')}`;
            } else if (serviceNames.length === 3) {
                title = `Combo Trío: ${serviceNames.join(' + ')}`;
            } else {
                title = `Mega Combo (${serviceNames.length} Servicios): ${serviceNames.join(' + ')}`;
            }
        ...
        // 3. Procesar Imagen (Lectura y compresión rápida cliente)
        ...
        const oldImage = (document.getElementById('comboOldImage')?.value || '').trim();
        let imageUrl = directUrl || oldImage;

        if (fileInput && fileInput.files && fileInput.files.length > 0) {
            try {
                const b64 = await compressImageFileToDataUrl(fileInput.files[0], 600, 350, 0.7);
                if (b64) imageUrl = b64;
            } catch(e){}
        }

        if (!imageUrl || imageUrl.trim() === '') {
            imageUrl = resolveProductImage({ title, category: 'Combos' });
        }
        ...
        const comboItem = {
            ...
            category: 'Combos',
            description: desc,
            price: Number(comboTotal) || 0,
            regularPriceTotal: Number(regularTotal) || 0,
            ...
            isOffer: true,
            isCombo: true,
            comboServices: cleanComboServices,
            advantages,
            imageUrl,
            updatedAt: new Date().toISOString()
        };

        // 4. Actualización instantánea en interfaz y memoria local
        if (!Array.isArray(appState.catalog)) appState.catalog = [];

        const isEdit = appState.catalog.some(c => c.id === id);
        const index = appState.catalog.findIndex(c => c.id === id);
        if (index > -1) {
            appState.catalog[index] = comboItem;
        } else {
            appState.catalog.push(comboItem);
        }

        serviceNames.forEach(sName => {
            if (!appState.services.some(s => s && s.toLowerCase() === sName.toLowerCase())) {
                appState.services.push(sName);
            }
        });
        ...
        const successMsg = isEdit 
            ? `🎉 ¡Combo "${title}" editado y publicado con éxito!`
            : `🎉 ¡Combo "${title}" creado y publicado con éxito!`;
        alert(successMsg);

        // 5. Guardado y sincronización en segundo plano con Firestore & Storage
        (async () => {
            try {
                if (statusLabel) statusLabel.innerHTML = '<span class="text-yellow-400"><i class="fa-solid fa-spinner fa-spin"></i> Sincronizando en la nube...</span>';

                // A) Guardar documento inmediatamente en Firestore
                await setDoc(doc(db, "store_catalog", id), comboItem, { merge: true });

                // B) Persistir servicios nuevos en Firestore
                for (const sName of serviceNames) {
                    try {
                        const servDocId = 'serv_' + sName.toLowerCase().replace(/[^a-z0-9]/g, '_');
                        await setDoc(doc(db, "services", servDocId), { name: sName }, { merge: true });
                    } catch(e){}
                }

                if (statusLabel) statusLabel.innerHTML = '<span class="text-emerald-400"><i class="fa-solid fa-cloud-check"></i> Sincronizado</span>';

                // C) Subir a Storage en segundo plano si hay archivo nuevo y actualizar URL
                if (fileInput && fileInput.files && fileInput.files.length > 0) {
                    try {
                        const file = fileInput.files[0];
                        const storageRef = ref(storage, 'catalogo/combos_' + Date.now() + '_' + file.name.replace(/[^a-zA-Z0-9._-]/g, '_'));
                        await uploadBytes(storageRef, file);
                        const cloudUrl = await getDownloadURL(storageRef);
                        if (cloudUrl) {
                            comboItem.imageUrl = cloudUrl;
                            const idx = appState.catalog.findIndex(c => c.id === id);
                            if (idx > -1) appState.catalog[idx].imageUrl = cloudUrl;
                            saveLocal();
                            window.renderCatalog();
                            await setDoc(doc(db, "store_catalog", id), { imageUrl: cloudUrl }, { merge: true });
                        }
                    } catch(errStorage) {
                        console.warn("Storage upload notice (usando imagen local optimizada):", errStorage);
                    }
                }
            } catch(errSync) {
                console.warn("Firestore background sync notice:", errSync);
                if (statusLabel) statusLabel.innerHTML = '<span class="text-emerald-400"><i class="fa-solid fa-cloud-check"></i> Guardado Local</span>';
            }
        })();

    } catch(errMain) {
        console.error("Error al procesar combo:", errMain);
        alert("⚠️ Ocurrió un error al procesar el combo: " + (errMain.message || errMain));
    } finally {
        if (saveBtn) {
            saveBtn.disabled = false;
...
window.deleteComboItem = async () => {
    ...
    if (confirm("¿Eliminar este Combo de la tienda definitivamente?")) {
        const statusLabel = document.getElementById('dbStatus');
        if (statusLabel) statusLabel.innerHTML = '<span class="text-yellow-400"><i class="fa-solid fa-spinner fa-spin"></i> Eliminando combo...</span>';
        ...
        try { 
            await deleteDoc(doc(db, "store_catalog", id)); 
            if (statusLabel) statusLabel.innerHTML = '<span class="text-emerald-400"><i class="fa-solid fa-cloud-check"></i> Sincronizado</span>';
            alert("🗑️ Combo eliminado de la tienda.");
        } catch(e){
            console.warn("Error eliminando combo de Firestore:", e);
        }
    }
};
...
window.toggleNewPostitForm = (show = null) => {
    ...
    if (!form.classList.contains('hidden')) {
        ...
        const saveBtn = document.getElementById('btnSavePostit');
...
window.editPostit = (id) => {
    ...
    const saveBtn = document.getElementById('btnSavePostit');
    if (saveBtn) saveBtn.innerHTML = '<i class="fa-solid fa-arrows-rotate mr-1"></i> Actualizar';
};
...
window.renderPostits = () => {
    ...
    sorted.forEach(p => {
        ...
        html += `
        <div class="border-l-4 ${borderClass} rounded-r-xl p-2.5 shadow-md relative group transition hover:brightness-110 space-y-1.5">
            <div class="flex items-center justify-between gap-1.5">
                <button type="button" onclick="window.togglePinPostit('${p.id}')" class="text-xs ${p.pinned ? pinColor : 'text-gray-500 hover:text-white'} transition" title="${p.pinned ? 'Desfijar' : 'Fijar arriba'}">
            ...
            <div class="flex items-center justify-end gap-1.5 pt-1 border-t border-gray-800/60 mt-1">
                <button type="button" onclick="window.editPostit('${p.id}')" class="bg-gray-900 hover:bg-gray-800 border border-gray-700 text-cuycito-gold hover:text-white text-[10px] font-bold px-2 py-0.5 rounded transition flex items-center gap-1 shadow cursor-pointer" title="Actualizar / Editar este Posit">
                    <i class="fa-solid fa-pen-to-square"></i> Actualizar
                </button>
                <button type="button" onclick="window.deletePostit('${p.id}')" class="text-gray-500 hover:text-red-400 text-[10px] p-0.5 transition" title="Eliminar nota"><i class="fa-solid fa-trash"></i></button>
            </div>
        </div>`;
    });
...
window.renderMasterAccounts = () => {
    ...
    const filter = masterActiveFilter || 'ALL';
    ...
    const allAccounts = appState.masterAccounts || [];
    allAccounts.forEach(acc => {
        const days = window.getDaysRemaining(acc.endDate);
        const isExpired = days < 0;
        totalAccounts++;
        if (isExpired) expiredAccounts++;
        else activeAccounts++;

        const costInPen = convertToGlobal(acc.cost || 0, acc.currency || 'PEN');
        totalMonthlyCost += costInPen;

        const occupied = (acc.profiles || []).filter(p => p !== null).length;
        totalFreeSlots += Math.max(0, (acc.capacity || 5) - occupied);
    });

    // Actualizar Tarjetas de Métricas Superior
    if (document.getElementById('masterStatTotal')) document.getElementById('masterStatTotal').innerText = totalAccounts;
    if (document.getElementById('masterStatActive')) document.getElementById('masterStatActive').innerText = activeAccounts;
    if (document.getElementById('masterStatExpired')) document.getElementById('masterStatExpired').innerText = expiredAccounts;
    if (document.getElementById('masterStatCost')) document.getElementById('masterStatCost').innerText = `S/ ${totalMonthlyCost.toFixed(2)}`;
    if (document.getElementById('masterStatFreeSlots')) document.getElementById('masterStatFreeSlots').innerText = totalFreeSlots;

    // Actualizar Badges de Filtros
    if (document.getElementById('countMasterFilter_ALL')) document.getElementById('countMasterFilter_ALL').innerText = totalAccounts;
    if (document.getElementById('countMasterFilter_ACTIVE')) document.getElementById('countMasterFilter_ACTIVE').innerText = activeAccounts;
    if (document.getElementById('countMasterFilter_EXPIRED')) document.getElementById('countMasterFilter_EXPIRED').innerText = expiredAccounts;
    ...
    let filtered = allAccounts.filter(acc => {
        ...
        const masterCode = (acc.masterCode || window.getMasterCode(acc)).toLowerCase();
        const service = (acc.service || '').toLowerCase();
        const email = (acc.email || '').toLowerCase();
        const provider = (acc.provider || '').toLowerCase();
        const providerPhone = (acc.providerPhone || '').toLowerCase();
        ...
        return !searchTerm || 
            ...
            providerPhone.includes(searchTerm);
    });
    ...
    if (filtered.length === 0) {
        ...
        <tr>
            <td colspan="7" class="text-center py-12 text-gray-500">
                <i class="fa-solid fa-server text-3xl text-gray-600 mb-2 block"></i>
                <p class="text-xs font-bold text-gray-400">No se encontraron Cuentas Matriz con el filtro seleccionado.</p>
            </td>
        </tr>`;
        window.renderMasterHistoryTable();
        return;
    }

    tbody.innerHTML = filtered.map(acc => {
        if (!acc.id) acc.id = `master_${Date.now()}`;
        const masterCode = acc.masterCode || window.getMasterCode(acc);
        acc.masterCode = masterCode;

        const days = window.getDaysRemaining(acc.endDate);
        const isExpired = days < 0;
        const endDateStr = window.formatDateDDMMYYYY(acc.endDate);

        let statusBadge = isExpired
            ? `<span class="bg-red-950 text-red-400 border border-cuycito-red/50 text-[10px] font-black px-2.5 py-1 rounded-lg shadow-sm flex items-center justify-center gap-1 w-fit mx-auto"><i class="fa-solid fa-triangle-exclamation"></i> VENCIDA</span>`
            : `<span class="bg-emerald-950 text-emerald-400 border border-emerald-500/40 text-[10px] font-black px-2.5 py-1 rounded-lg shadow-sm flex items-center justify-center gap-1 w-fit mx-auto"><i class="fa-solid fa-circle-check"></i> ACTIVA (${days}d)</span>`;

        const occupied = (acc.profiles || []).filter(p => p !== null).length;
        const freeSlots = Math.max(0, (acc.capacity || 5) - occupied);
        const occBadgeClass = occupied === acc.capacity
            ? 'bg-cuycito-red/20 text-red-400 border-cuycito-red/50'
        ...
        const passVisibilityBtn = isCredsVisible
            ? `<button onclick="window.toggleMasterCredentialsVisibility('${acc.id}')" class="bg-emerald-950 text-emerald-400 hover:bg-emerald-900 border border-emerald-500/40 text-[10px] font-bold px-2.5 py-1 rounded-lg transition flex items-center gap-1 mx-auto cursor-pointer" title="Credenciales Visibles al cliente"><i class="fa-solid fa-eye text-xs"></i> Visibles</button>`
            : `<button onclick="window.toggleMasterCredentialsVisibility('${acc.id}')" class="bg-red-950 text-red-400 hover:bg-red-900 border border-cuycito-red/50 text-[10px] font-bold px-2.5 py-1 rounded-lg transition flex items-center gap-1 mx-auto cursor-pointer" title="Credenciales Ocultas al cliente"><i class="fa-solid fa-eye-slash text-xs"></i> Ocultas</button>`;

        const cleanPhone = (acc.providerPhone || '').replace(/\D/g, '');
        const whatsappBtn = cleanPhone
            ? `<a href="https://wa.me/${cleanPhone.startsWith('51') ? cleanPhone : '51' + cleanPhone}" target="_blank" class="text-emerald-400 hover:text-emerald-300 transition text-[11px] font-mono font-bold flex items-center gap-1 mt-0.5"><i class="fa-brands fa-whatsapp"></i> ${acc.providerPhone}</a>`
            : (acc.providerPhone ? `<span class="text-[10px] text-gray-400 font-mono block">${acc.providerPhone}</span>` : '');

        const isExpanded = window.expandedMasterSlots.has(acc.id);
        const expandSlotsBtnText = isExpanded ? '▲ Ocultar Cupos' : `▼ Ver Cupos (${occupied}/${acc.capacity})`;

        let slotsHtml = '';
        if (isExpanded) {
            slotsHtml += `<tr id="slots_row_${acc.id}" class="bg-black/90"><td colspan="7" class="p-4 border-b border-gray-800">
                <div class="space-y-2 max-w-4xl mx-auto">
                    <div class="flex justify-between items-center border-b border-gray-800 pb-2">
                        <span class="text-xs font-black text-cuycito-gold uppercase"><i class="fa-solid fa-users mr-1.5"></i> Cupos Asignados - ${acc.service} [${masterCode}] (${acc.email})</span>
                        <span class="text-[10px] text-gray-400">Capacidad Total: ${acc.capacity} clientes</span>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 pt-1">`;

            for (let i = 0; i < (acc.capacity || 5); i++) {
                const subId = (acc.profiles || [])[i];
                if (subId) {
                    const sub = appState.subscriptions.find(s => s.id === subId);
                    if (sub) {
                        const dColor = window.getDaysRemaining(sub.endDate) < 0 ? 'text-red-400' : 'text-emerald-400';
                        const subSym = sub.currency === 'USD' ? '$' : 'S/';
                        slotsHtml += `
                        <div class="flex justify-between items-center bg-[#141414] border border-gray-700 rounded-xl p-2.5 shadow">
                            <div>
                                <p class="text-xs font-black text-white truncate">${sub.person} <span class="text-[10px] text-cuycito-gold font-mono">(${sub.pin || 'Perfil ' + (i+1)})</span></p>
                                <p class="text-[10px] ${dColor} font-mono">Vence: ${window.formatDateDDMMYYYY(sub.endDate)} | Cobra: ${subSym}${sub.amount}</p>
                            </div>
                            <button onclick="window.unlinkProfile('${acc.id}', ${i})" class="text-gray-400 hover:text-cuycito-red transition text-xs p-1.5 cursor-pointer" title="Desvincular Cliente"><i class="fa-solid fa-xmark"></i></button>
                        </div>`;
                    } else { acc.profiles[i] = null; }
                }
                if (!subId) {
                    slotsHtml += `
                    <div onclick="window.openAssignModal('${acc.id}', ${i})" class="flex justify-center items-center border border-dashed border-gray-700 bg-black/40 rounded-xl p-2.5 cursor-pointer hover:border-cuycito-gold hover:bg-cuycito-gold/10 transition group h-12">
                        <span class="text-xs text-gray-500 font-bold group-hover:text-cuycito-gold"><i class="fa-solid fa-plus mr-1"></i> Cupo #${i+1}: Asignar Cliente</span>
                    </div>`;
                }
            }
            slotsHtml += `</div></div></td></tr>`;
        }
        ...
        <tr class="hover:bg-black/40 transition">
            ...
            <td class="p-3.5 align-middle">
                <span class="bg-amber-950 text-cuycito-gold border border-amber-500/40 text-[10px] font-black px-2 py-1 rounded-lg font-mono inline-block shadow-sm">${masterCode}</span>
            </td>

            <!-- Servicio & Cuenta -->
            <td class="p-3.5 align-middle">
                <div class="font-bold text-white text-xs uppercase">${acc.service}</div>
                <div class="text-[11px] text-cuycito-gold font-mono truncate max-w-[220px]">${acc.email}</div>
                <div class="text-[9px] text-gray-500 font-mono">Pass: ${acc.pass}</div>
            </td>

            <!-- Proveedor & Contacto -->
            <td class="p-3.5 align-middle">
                <div class="text-xs font-bold text-gray-200">${acc.provider || 'Sin proveedor'}</div>
                ${whatsappBtn}
            </td>

            <!-- Costo & Ocupación -->
            <td class="p-3.5 align-middle text-center">
                <div class="text-xs font-black text-red-400 font-mono">${acc.currency === 'USD' ? '$' : 'S/'} ${parseFloat(acc.cost || 0).toFixed(2)}</div>
                <div class="mt-1">
                    <span class="border px-2 py-0.5 rounded-md ${occBadgeClass} text-[9px] font-black tracking-wider inline-block">${occupied}/${acc.capacity} Lleno (${freeSlots} libres)</span>
                </div>
            </td>

            <!-- Vencimiento & Estado -->
            <td class="p-3.5 align-middle text-center">
                <div class="text-xs font-bold text-gray-200 font-mono mb-1">${endDateStr}</div>
                ${statusBadge}
            </td>
            ...
            <td class="p-3.5 align-middle text-center">
                <div class="flex items-center justify-center gap-1.5">
                    <button onclick="window.openRenewMasterModal('${acc.id}')" class="bg-emerald-950/80 hover:bg-emerald-900 border border-emerald-500/40 text-emerald-400 p-1.5 rounded-lg transition cursor-pointer" title="Renovar Cuenta Matriz">
                        <i class="fa-solid fa-rotate text-xs"></i>
                    </button>
                    <button onclick="window.openEditMasterModal('${acc.id}')" class="bg-blue-950/80 hover:bg-blue-900 border border-blue-500/40 text-blue-400 p-1.5 rounded-lg transition cursor-pointer" title="Editar Datos">
                        <i class="fa-solid fa-pen-to-square text-xs"></i>
                    </button>
                    <button onclick="window.deleteMasterAccount('${acc.id}')" class="bg-red-950/80 hover:bg-red-900 border border-cuycito-red/50 text-red-400 p-1.5 rounded-lg transition cursor-pointer" title="Eliminar Cuenta Matriz">
                        <i class="fa-solid fa-trash text-xs"></i>
                    </button>
                    <button onclick="window.toggleMasterSlots('${acc.id}')" class="bg-gray-900 hover:bg-gray-800 border border-gray-700 text-gray-300 text-[10px] font-bold px-2 py-1 rounded-lg transition cursor-pointer" title="Ver / Ocultar Cupos de Clientes">
                        ${expandSlotsBtnText}
                    </button>
                </div>
            </td>
...
window.renderMasterHistoryTable = () => {
    ...
    const masterEvents = history.filter(h => {
        const type = (h.type || '').toUpperCase();
        const serv = (h.service || '').toLowerCase();
        return type.includes('MATRIZ') || type === 'COMPRA' || serv.includes('matriz') || serv.includes('mat-') || h.masterAccountId;
    });
    ...
    if (masterEvents.length === 0) {
        ...
        <tr>
            <td colspan="5" class="text-center py-6 text-gray-500">
                Sin movimientos ni renovaciones de cuentas matriz registrados.
            </td>
        </tr>`;
        return;
    }

    const sorted = [...masterEvents].reverse();

    tbody.innerHTML = sorted.map(tx => {
        const dateStr = tx.date || window.formatDateDDMMYYYY(new Date());
        let typeBadge = '<span class="bg-blue-950 text-blue-300 border border-blue-500/40 text-[9px] font-black px-2 py-0.5 rounded">REGISTRO</span>';

        if (tx.type === 'COMPRA') {
            typeBadge = '<span class="bg-purple-950 text-purple-300 border border-purple-500/40 text-[9px] font-black px-2 py-0.5 rounded">COMPRA INICIAL</span>';
        } else if (tx.type === 'RENOVACION_MATRIZ') {
            typeBadge = '<span class="bg-emerald-950 text-emerald-300 border border-emerald-500/40 text-[9px] font-black px-2 py-0.5 rounded">RENOVACIÓN</span>';
        } else if (tx.type === 'CAMBIO_FECHA_MATRIZ') {
            typeBadge = '<span class="bg-amber-950 text-amber-300 border border-amber-500/40 text-[9px] font-black px-2 py-0.5 rounded">EDICIÓN FECHA</span>';
        } else if (tx.type === 'BAJA_MATRIZ') {
            typeBadge = '<span class="bg-red-950 text-red-300 border border-cuycito-red/50 text-[9px] font-black px-2 py-0.5 rounded">ELIMINACIÓN</span>';
        }

        const amt = parseFloat(tx.amount || 0);
        const amtStr = amt > 0 ? `${tx.currency === 'USD' ? '$' : 'S/'} ${amt.toFixed(2)}` : 'S/ 0.00';

        return `
        <tr class="hover:bg-black/30 transition">
            <td class="p-3 text-gray-400">${dateStr}</td>
            <td class="p-3">${typeBadge}</td>
            <td class="p-3 font-bold text-white">${tx.service || 'Cuenta Matriz'}</td>
            <td class="p-3 text-gray-300">${tx.person || 'Proveedor'}</td>
            <td class="p-3 text-right font-bold ${amt > 0 ? 'text-red-400' : 'text-gray-500'}">${amtStr}</td>
        </tr>`;
    }).join('');
};
...
window.renderClients = () => {
    ...
    if (filteredClients.length === 0) {
        tbody.innerHTML = `<tr><td colspan="8" class="p-8 text-center text-gray-500 italic">No se encontraron accesos de clientes.</td></tr>`;
        return;
    }
    ...
    filteredClients.forEach(c => {
        ...
        // Contador de inicios de sesión del cliente (excluye supervisiones admin)
        const loginCount = parseInt(c.loginCount) || 0;
        let lastLoginText = 'Sin ingresos registrados';
        let lastLoginShort = '-';
        if (c.lastLoginAt) {
            try {
                const dateObj = new Date(c.lastLoginAt);
                lastLoginText = dateObj.toLocaleString('es-PE', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
                lastLoginShort = dateObj.toLocaleDateString('es-PE', { day: '2-digit', month: '2-digit' }) + ' ' + dateObj.toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit' });
            } catch(e){}
        }

        const loginBadgeHTML = loginCount > 0 
            ? `<div class="flex flex-col items-center justify-center">
                 <span class="bg-purple-950/80 text-purple-300 border border-purple-500/50 text-xs font-black px-2.5 py-0.5 rounded-lg flex items-center gap-1 shadow" title="Total de inicios de sesión: ${loginCount}\nÚltimo acceso del cliente: ${lastLoginText}\n(Excluye supervisiones de administrador)">
                     <i class="fa-solid fa-door-open text-[11px] text-purple-400"></i>
                     <span>${loginCount}</span>
                 </span>
                 <span class="text-[9px] text-gray-500 font-mono mt-0.5" title="Último ingreso: ${lastLoginText}">${lastLoginShort}</span>
               </div>`
            : `<div class="flex flex-col items-center justify-center text-gray-600 font-mono text-[11px]" title="El cliente aún no ha iniciado sesión por su cuenta">
                 <span class="bg-gray-900 border border-gray-800 text-gray-500 text-xs font-bold px-2 py-0.5 rounded-lg flex items-center gap-1">
                     <i class="fa-solid fa-door-closed text-[10px] text-gray-600"></i>
                     <span>0</span>
                 </span>
               </div>`;
        ...
        <tr class="hover:bg-gray-800/60 transition">
            ...
            <td class="p-4 text-center">
                <div class="flex items-center justify-center gap-1.5">
                    <button onclick="window.impersonateAndOpenProfile('${c.id}')" class="bg-gradient-to-r from-purple-900 to-indigo-800 hover:from-purple-700 hover:to-indigo-600 text-white font-black p-2 px-2.5 rounded-lg transition shadow text-xs flex items-center gap-1 cursor-pointer" title="Entrar y supervisar el perfil de este cliente como Administrador (Sin sumar contador de visitas)">
                        <i class="fa-solid fa-arrow-up-right-from-square text-xs"></i> <span class="hidden xl:inline">Supervisar</span>
                    </button>
                    <button onclick="window.openClientServicesModal('${c.id}')" class="bg-cuycito-gold hover:bg-cuycito-goldHover text-black font-black p-2 px-2.5 rounded-lg transition shadow text-xs flex items-center gap-1 cursor-pointer" title="Gestionar y Enlazar Servicios">
                        <i class="fa-solid fa-link"></i> <span class="hidden lg:inline">Enlazar</span>
                    </button>
                    <button onclick="window.copyClientAccess('${c.id}')" class="bg-indigo-600 hover:bg-indigo-500 text-white p-2 rounded-lg transition shadow cursor-pointer" title="Copiar mensaje con credenciales">
                        <i class="fa-regular fa-copy text-xs"></i>
                    </button>
                    <button onclick="window.sendClientAccessWhatsApp('${c.id}')" class="bg-[#25D366] hover:bg-emerald-500 text-black p-2 rounded-lg transition shadow cursor-pointer" title="Enviar mensaje por WhatsApp">
                        <i class="fa-brands fa-whatsapp text-xs font-bold"></i>
                    </button>
                    <button onclick="window.openClientModal('${c.id}')" class="bg-gray-700 hover:bg-blue-600 text-white p-2 rounded-lg transition shadow cursor-pointer" title="Editar datos">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-gray-400 font-mono mt-0.5">
                        ...
                        | 🚪 Entradas: <span class="text-purple-300 font-bold">${loginCount}</span>
                        ${c.email ? ` | 📧 ${c.email}` : ''}
                    </p>
                </div>
            </div>
            <button type="button" onclick="window.impersonateAndOpenProfile('${c.id}')" class="bg-gradient-to-r from-purple-900 to-indigo-800 hover:from-purple-700 hover:to-indigo-600 text-white font-black text-xs px-3.5 py-1.5 rounded-lg transition shadow flex items-center gap-1.5 cursor-pointer">
...
window.impersonateAndOpenProfile = (clientId) => {
    ...
    // Configurar sesión supervisada EXCLUSIVA DE ADMINISTRADOR
    const clientSession = {
        id: client.id,
        name: client.name,
    ...
    // Abrir perfil en una nueva pestaña con parámetro explícito de supervisión admin
...
window.renderNotifications = () => {
    ...
    // 1. Cuentas Matrices por vencer (Aviso a partir de 10 días o menos, omitiendo noRenovar)
    const expiringMasters = (appState.masterAccounts || []).filter(acc => {
        if (!acc.endDate || acc.noRenovar) return false;
        const days = window.getDaysRemaining(acc.endDate);
        return days <= 10;
    }).map(acc => {
        const days = window.getDaysRemaining(acc.endDate);
        return {
            type: 'MASTER',
            id: acc.id,
            title: acc.service || 'Servicio',
            provider: acc.provider || acc.providerPhone || acc.email || 'Proveedor',
            subtitle: acc.email,
            endDate: acc.endDate,
            days: days,
            isExpired: days < 0,
    ...
    // 2. Suscripciones de Clientes por vencer (Aviso a partir de 10 días o menos, omitiendo noRenovar)
    const expiringSubs = (appState.subscriptions || []).filter(s => {
        if (!s.endDate || s.status === 'pending_activation' || s.noRenovar) return false;
        const days = window.getDaysRemaining(s.endDate);
        return days <= 10;
    }).map(s => {
        const days = window.getDaysRemaining(s.endDate);
        const client = (appState.clients || []).find(c => 
            (s.clientId && c.id === s.clientId) || 
            (s.clientCode && (c.clientCode || window.getClientCode(c)) === s.clientCode) ||
            (s.person && (c.name || '').trim().toLowerCase() === (s.person || '').trim().toLowerCase())
        );
        const phone = (s.clientPhone || s.phone || (client ? client.phone : '') || '').replace(/\D/g, '');
        const clientKey = (s.clientId || s.clientCode || s.person || 'cliente').toString().trim().toLowerCase();
    ...
    // Contar servicios por cliente para mensajes de renovación múltiple
    const clientExpiringCounts = {};
    expiringSubs.forEach(s => {
        clientExpiringCounts[s.clientKey] = (clientExpiringCounts[s.clientKey] || 0) + 1;
    });
    ...
    allAlerts.forEach(item => {
        ...
        const tagHTML = isMaster
            ? `<span class="bg-purple-950 text-purple-300 border border-purple-500/50 text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider flex items-center gap-1 shadow">
                 <i class="fa-solid fa-crown text-[8px] text-yellow-400"></i> M P
               </span>`
            : `<span class="bg-blue-950/80 text-blue-300 border border-blue-500/40 text-[9px] font-bold px-1.5 py-0.5 rounded">
                 👤 Cliente
               </span>`;

        // Acciones para Cuentas Matriz
        const masterActionsHTML = isMaster ? `
            <div class="flex items-center justify-between pt-1.5 border-t border-gray-800/80 mt-1 flex-wrap gap-1">
                <span class="text-[9px] text-purple-400 font-bold flex items-center gap-1">
                    <i class="fa-solid fa-crown text-[8px]"></i> Matriz
                </span>
                <div class="flex items-center gap-1 flex-wrap">
                    <button type="button" onclick="window.openRenewMasterModal('${item.id}')" class="bg-purple-900/40 hover:bg-purple-900 text-purple-300 border border-purple-500/40 px-1.5 py-0.5 rounded text-[10px] font-bold transition flex items-center gap-1 shadow cursor-pointer" title="Renovar cuenta matriz">
                        <i class="fa-solid fa-arrows-rotate text-[10px]"></i>
                        <span>Renovar</span>
                    </button>
                    <button type="button" onclick="window.viewMasterAccountFromAlert('${item.id}')" class="bg-gray-900 hover:bg-gray-800 border border-gray-700 text-gray-300 hover:text-white px-1.5 py-0.5 rounded text-[10px] font-bold transition flex items-center gap-1 shadow cursor-pointer" title="Visualizar en Cuentas Raíz">
                        <i class="fa-regular fa-eye text-cuycito-gold text-[10px]"></i>
                        <span>Visualizar</span>
                    </button>
                    <button type="button" onclick="window.toggleNoRenovarMaster('${item.id}')" class="bg-red-950/40 hover:bg-red-900 text-red-300 border border-red-500/40 px-1.5 py-0.5 rounded text-[10px] font-bold transition flex items-center gap-1 shadow cursor-pointer" title="Marcar como No Renovar (deja de mostrar en Por Vencer)">
        ...
        // Acciones para Clientes (con soporte para mensaje múltiple si tiene > 1 servicio por vencer)
        const multiCount = item.clientKey ? (clientExpiringCounts[item.clientKey] || 1) : 1;
        const clientActionsHTML = !isMaster ? `
            <div class="flex items-center justify-between pt-1.5 border-t border-gray-800/80 mt-1 flex-wrap gap-1">
                <span class="text-[9px] text-yellow-400/80 font-bold flex items-center gap-1">
                    <i class="fa-solid fa-bell text-[8px]"></i> ${isExpired ? 'Servicio vencido' : 'Solicitar renovación'}
                </span>
                <div class="flex items-center gap-1 flex-wrap">
                    ${multiCount > 1 ? `
                    <button type="button" onclick="window.copyMultiRenewalMessage('${item.clientKey}')" class="bg-amber-950/80 hover:bg-amber-900 border border-amber-500/60 text-amber-300 px-1.5 py-0.5 rounded text-[10px] font-bold transition flex items-center gap-1 shadow cursor-pointer" title="Copiar mensaje conciso con todos sus servicios (${multiCount})">
                        <i class="fa-solid fa-layer-group text-cuycito-gold text-[10px]"></i>
                        <span>Copiar Todo (${multiCount})</span>
                    </button>
                    ${item.phone ? `
                    <button type="button" onclick="window.sendMultiRenewalWhatsApp('${item.clientKey}')" class="bg-[#25D366]/30 hover:bg-[#25D366] text-[#25D366] hover:text-black border border-[#25D366]/50 px-1.5 py-0.5 rounded text-[10px] font-bold transition flex items-center gap-1 shadow cursor-pointer" title="Enviar WhatsApp con todos sus servicios (${multiCount})">
                        <i class="fa-brands fa-whatsapp text-xs"></i>
                        <span>WSP Todo (${multiCount})</span>
                    </button>` : ''}
                    ` : `
                    <button type="button" onclick="window.copyRenewalMessage('${item.id}')" class="bg-gray-900 hover:bg-gray-800 border border-gray-700 text-gray-300 hover:text-white px-2 py-0.5 rounded text-[10px] font-bold transition flex items-center gap-1 shadow cursor-pointer" title="Copiar mensaje de recordatorio de renovación">
                        <i class="fa-regular fa-copy text-cuycito-gold text-[10px]"></i>
                        <span>Copiar</span>
                    </button>
                    ${item.phone ? `
                    <button type="button" onclick="window.sendRenewalWhatsApp('${item.id}')" class="bg-[#25D366]/20 hover:bg-[#25D366] text-[#25D366] hover:text-black border border-[#25D366]/40 px-2 py-0.5 rounded text-[10px] font-bold transition flex items-center gap-1 shadow cursor-pointer" title="Enviar recordatorio por WhatsApp">
                        <i class="fa-brands fa-whatsapp text-xs"></i>
                        <span>WhatsApp</span>
                    </button>` : ''}
                    `}
                    <button type="button" onclick="window.toggleNoRenovarSub('${item.id}')" class="text-gray-500 hover:text-red-400 p-0.5 transition" title="Ocultar de Por Vencer">
        ...
        const displaySubtitle = isMaster ? (item.provider || item.subtitle || 'Proveedor') : item.subtitle;
        const formattedEndDate = window.formatDateDDMMYYYY ? window.formatDateDDMMYYYY(item.endDate) : item.endDate;
        ...
        <div class="p-3 bg-[#0a0a0a] border-l-4 ${borderClass} shadow-lg rounded-r-xl space-y-1 hover:bg-[#111] transition">
            ...
            <div class="flex justify-between items-center text-[11px] text-gray-400 font-mono">
                <span class="truncate max-w-[150px]">${displaySubtitle}</span>
                <span class="text-[10px] text-gray-500">Vence: ${formattedEndDate}</span>
            </div>
            ${isMaster ? masterActionsHTML : clientActionsHTML}
        </div>`;
    });
};

// =====================================
// FUNCIONES DE ACCIÓN PARA ALERTAS (MATRIZ Y MENSAJES MÚLTIPLES)
// =====================================
window.viewMasterAccountFromAlert = (accId) => {
    if (window.switchTab) window.switchTab('master');
    const searchInput = document.getElementById('masterSearchInput');
    const acc = (appState.masterAccounts || []).find(m => m.id === accId);
    if (searchInput && acc) {
        searchInput.value = acc.service || accId;
        if (window.renderMasterAccounts) window.renderMasterAccounts();
    }
    setTimeout(() => {
        const row = document.getElementById(`master-row-${accId}`);
        if (row) {
            row.scrollIntoView({ behavior: 'smooth', block: 'center' });
            row.classList.add('ring-2', 'ring-cuycito-gold', 'bg-cuycito-gold/10');
            setTimeout(() => row.classList.remove('ring-2', 'ring-cuycito-gold', 'bg-cuycito-gold/10'), 2500);
        }
    }, 200);
};

window.toggleNoRenovarMaster = async (accId) => {
    const acc = (appState.masterAccounts || []).find(m => m.id === accId);
    if (!acc) return;
    if (confirm(`¿Marcar la cuenta matriz "${acc.service}" como "No renovar"? Dejará de mostrarse en la lista de Por Vencer.`)) {
        acc.noRenovar = true;
        try {
            await setDoc(doc(db, "masterAccounts", accId), { noRenovar: true }, { merge: true });
        } catch(e) { console.error("Error guardando noRenovar en masterAccounts:", e); }
        window.renderNotifications();
        if (window.renderMasterAccounts) window.renderMasterAccounts();
    }
};

window.toggleNoRenovarSub = async (subId) => {
    const sub = (appState.subscriptions || []).find(s => s.id === subId);
    if (!sub) return;
    if (confirm(`¿Ocultar la suscripción de "${sub.service}" (${sub.person}) de la lista de Por Vencer?`)) {
        sub.noRenovar = true;
        try {
            await setDoc(doc(db, "subscriptions", subId), { noRenovar: true }, { merge: true });
        } catch(e) { console.error("Error guardando noRenovar en subscriptions:", e); }
        window.renderNotifications();
        if (window.renderSubscriptions) window.renderSubscriptions();
    }
};

window.getMultiRenewalMessage = (clientKey) => {
    const clientSubs = (appState.subscriptions || []).filter(s => {
        if (!s.endDate || s.status === 'pending_activation' || s.noRenovar) return false;
        const days = window.getDaysRemaining(s.endDate);
        if (days > 10) return false;
        const cKey = (s.clientId || s.clientCode || s.person || 'cliente').toString().trim().toLowerCase();
        return cKey === clientKey;
    });

    if (clientSubs.length === 0) return '';

    const firstSub = clientSubs[0];
    const clientName = firstSub.person || 'Cliente';

    let servicesList = '';
    clientSubs.forEach(s => {
        const days = window.getDaysRemaining(s.endDate);
        const endDateFormatted = window.formatDateDDMMYYYY(s.endDate);
        const pinText = s.pin ? ` (PIN: ${s.pin})` : '';
        let statusText = '';
        if (days < 0) statusText = 'EXPIRÓ';
        else if (days === 0) statusText = 'VENCE HOY';
        else statusText = `Vence en ${days} días`;

        servicesList += `• *${s.service}*${pinText}: ${statusText} (${endDateFormatted})\n`;
    });

    return `¡Hola ${clientName}! 🐹✨\n\nTe escribimos de CuycitoGo para recordarte el estado de tus suscripciones:\n\n${servicesList}\n💡 Puedes realizar la renovación de tus servicios en conjunto para mantener tu beneficio activo sin interrupciones ni cortes de perfil.\n\n👉 Portal de cliente: https://cuycitogo.online/\n\n¡Quedamos atentos a tu confirmación para renovarte! 🙌`;
};

window.copyMultiRenewalMessage = async (clientKey) => {
    const msg = window.getMultiRenewalMessage(clientKey);
    if (!msg) return alert("No se encontraron servicios por vencer para este cliente.");
    try {
        ...
        alert("📋 ¡Mensaje de renovación múltiple copiado al portapapeles con éxito!\n\nPuedes pegarlo directamente en WhatsApp o chat con el cliente.");
    } catch(e) {
        prompt("Copia el siguiente mensaje de renovación múltiple:", msg);
    }
};

window.sendMultiRenewalWhatsApp = (clientKey) => {
    const clientSubs = (appState.subscriptions || []).filter(s => {
        if (!s.endDate || s.status === 'pending_activation' || s.noRenovar) return false;
        const days = window.getDaysRemaining(s.endDate);
        if (days > 10) return false;
    ...
    const firstSub = clientSubs[0];
    const client = (appState.clients || []).find(c => 
        (firstSub.clientId && c.id === firstSub.clientId) || 
        (firstSub.clientCode && (c.clientCode || window.getClientCode(c)) === firstSub.clientCode) ||
        (firstSub.person && (c.name || '').trim().toLowerCase() === (firstSub.person || '').trim().toLowerCase())
    );
    const phone = (firstSub.clientPhone || firstSub.phone || (client ? client.phone : '') || '').replace(/\D/g, '');
    const msg = window.getMultiRenewalMessage(clientKey);

    if (!phone) {
        alert("Este cliente no tiene un número de celular registrado para WhatsApp.");
        window.copyMultiRenewalMessage(clientKey);
        return;
    }
...
window.getRenewalReminderMessage = (subId) => {
    const sub = (appState.subscriptions || []).find(s => s.id === subId);
    if (!sub) return '';

    const client = (appState.clients || []).find(c => 
        (sub.clientId && c.id === sub.clientId) || 
        (sub.clientCode && (c.clientCode || window.getClientCode(c)) === sub.clientCode) ||
        (sub.person && (c.name || '').trim().toLowerCase() === (sub.person || '').trim().toLowerCase())
    );

    const clientName = sub.person || (client ? client.name : 'Cliente');
    const serviceName = sub.service || 'Streaming';
    const endDateFormatted = window.formatDateDDMMYYYY(sub.endDate);
    const days = window.getDaysRemaining(sub.endDate);
    const pinText = sub.pin ? ` (${sub.pin})` : '';
    const sym = (sub.currency === 'USD') ? '$' : 'S/';
    const priceText = sub.amount ? ` por ${sym} ${parseFloat(sub.amount).toFixed(2)}` : '';

    if (days < 0) {
        return `¡Hola ${clientName}! 🐹✨\n\nTe recordamos que tu suscripción de *${serviceName}*${pinText} ha *VENCIDO* el día *${endDateFormatted}*.\n\n💡 Puedes renovar tu cuenta${priceText} hoy mismo para reactivar tu servicio sin perder tus perfiles ni configuraciones.\n\n👉 Portal de cliente: https://cuycitogo.online/\n\n¡Quedamos atentos a tu confirmación para renovarte! 🙌`;
    } else if (days === 0) {
        return `¡Hola ${clientName}! 🐹✨\n\nTe recordamos que tu suscripción de *${serviceName}*${pinText} *VENCE HOY* (${endDateFormatted}).\n\n💡 Para continuar disfrutando de tu servicio sin cortes, puedes realizar tu renovación${priceText} ahora.\n\n👉 Portal de cliente: https://cuycitogo.online/\n\n¡Gracias por tu preferencia! 🙌`;
    } else {
        const diasText = days === 1 ? '1 día' : `${days} días`;
        return `¡Hola ${clientName}! 🐹✨\n\nTe recordamos que tu suscripción de *${serviceName}*${pinText} está próxima a vencer en *${diasText}* (el *${endDateFormatted}*).\n\n💡 Puedes realizar tu renovación${priceText} con anticipación para mantener tu servicio 100% activo sin interrupciones.\n\n👉 Portal de cliente: https://cuycitogo.online/\n\n¡Muchas gracias por tu preferencia! 🙌`;
    }
};

window.copyRenewalMessage = async (subId) => {
    const msg = window.getRenewalReminderMessage(subId);
    if (!msg) {
        alert("No se encontró la información de este servicio.");
        return;
    }
    ...
    try {
        ...
        alert("📋 ¡Mensaje de recordatorio de renovación copiado al portapapeles con éxito!\n\nPuedes pegarlo directamente en WhatsApp o chat con el cliente.");
    } catch(e) {
        prompt("Copia el siguiente mensaje de recordatorio:", msg);
    }
};

window.sendRenewalWhatsApp = (subId) => {
    const sub = (appState.subscriptions || []).find(s => s.id === subId);
    if (!sub) return;
    ...
    const phone = (sub.clientPhone || sub.phone || (client ? client.phone : '') || '').replace(/\D/g, '');
    const msg = window.getRenewalReminderMessage(subId);

    if (!phone) {
        alert("Este cliente no tiene un número de celular registrado para WhatsApp.");
        window.copyRenewalMessage(subId);
        return;
    }
