...
// ==========================================================================
let isStoreInMaintenance = localStorage.getItem('cuycito_store_maintenance') === 'true';

function applyMaintenanceUI(isM) {
    isStoreInMaintenance = isM;
...
// Ejecución inmediata
applyMaintenanceUI(isStoreInMaintenance);

// Listener Cross-Tab
window.addEventListener('storage', (e) => {
    if (e.key === 'cuycito_store_maintenance') {
        applyMaintenanceUI(e.newValue === 'true');
    }
});

// Listener Firestore Cloud
try {
...
if (form) {
    form.addEventListener('submit', async (e) => {
        ...
        // BLOQUEO POR PROTOCOLO DE MANTENIMIENTO ACTIVO
        if (isStoreInMaintenance) {
            alertBox.innerText = '🛑 El acceso por login se encuentra pausado temporalmente debido a labores de mantenimiento técnico en la tienda. Intenta nuevamente cuando el servicio vuelva al estado operativo.';
