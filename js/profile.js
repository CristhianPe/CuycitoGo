...
function getSubscriptionPrice(sub) {
    if (!sub) return 15.00;
    // 1. Si la suscripción ya tiene un precio asignado en la base de datos (conservar lo que ya se le cobra en Servicio Activo)
...
async function initProfilePage() {
    ...
    const isSupervised = isAdminSupervision || (currentClientUser && currentClientUser.isSupervisedByAdmin);
    const isDemoAccount = !currentClientUser || currentClientUser.isDemo || currentClientUser.id === 'demo_cuycito_user' || currentClientUser.phone === 'cuycitogodemo';
    ...
    } else if (!isDemoAccount && currentClientUser && currentClientUser.id) {
        // Contabilizar ingreso del cliente auténtico (Solo una vez por sesión de pestaña activa)
        const sessionCountedKey = "cuycito_client_session_counted_" + currentClientUser.id;
        if (!sessionStorage.getItem(sessionCountedKey)) {
            sessionStorage.setItem(sessionCountedKey, "true");
            const newCount = (parseInt(currentClientUser.loginCount) || 0) + 1;
            const nowIso = new Date().toISOString();
            currentClientUser.loginCount = newCount;
            currentClientUser.lastLoginAt = nowIso;
            localStorage.setItem("cuycitoClient", JSON.stringify(currentClientUser));
            ...
            try {
                ...
                console.warn("No se pudo actualizar loginCount en Firestore:", errLogCount);
            }
        }
    }
...
function isSubscriptionBelongingToClient(sub, client) {
    ...
    const normalizeStr = (s) => (s || '').toString().trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");

    const clientId = (client.id || '').toString().trim();
    const clientCode = normalizeCode(client.clientCode || (typeof getClientCode === 'function' ? getClientCode(client) : ''));
    const clientPhone = normalizePhone(client.phone);
    const clientName = normalizeStr(client.name);
    const clientNick = normalizeStr(client.nickname);

    const subClientId = (sub.clientId || sub.userId || '').toString().trim();
    const subClientCode = normalizeCode(sub.clientCode);
    const subPhone = normalizePhone(sub.clientPhone || sub.phone);
    const subPerson = normalizeStr(sub.person || sub.clientName || sub.name);

    // 0. Cuenta Demo
    if (client.isDemo && sub.isDemo) return true;

    const isGenericCode = (code) => !code || code === 'CLI-000' || code === 'CLI-1001' || code === 'CLI-CLIENT';
    const phonesMatch = (p1, p2) => {
        if (!p1 || !p2 || p1.length < 7 || p2.length < 7) return false;
        return p1 === p2 || p1.endsWith(p2) || p2.endsWith(p1);
    };

    // 1. REGLA ESTRICTA DE RECHAZO: Solo rechazar si pertenece explícitamente a OTRO cliente por conflicto de ID, Código o Teléfono
    if (subClientId && clientId && subClientId !== clientId && subClientId !== clientCode) {
        if (!clientPhone || (clientPhone.length >= 7 && !subClientId.includes(clientPhone) && !clientPhone.includes(subClientId))) {
            return false;
        }
    }

    if (!isGenericCode(subClientCode) && clientCode && !isGenericCode(clientCode) && subClientCode !== clientCode && subClientCode !== clientId) {
        return false;
    }

    if (subPhone && subPhone.length >= 7 && clientPhone && clientPhone.length >= 7 && !phonesMatch(subPhone, clientPhone)) {
        if (!subClientId || subClientId !== clientId) {
            return false;
        }
    }

    // 2. COINCIDENCIAS POSITIVAS
    // A) Coincidencia directa por ID Principal de Firestore o Código
    if (subClientId && (subClientId === clientId || subClientId === clientCode)) {
        return true;
    }

    // B) Coincidencia directa por Código Único CLI-XXXX
    if (!isGenericCode(subClientCode) && (subClientCode === clientCode || subClientCode === clientId)) {
        return true;
    }

    // C) Coincidencia directa por Número Telefónico (con o sin código de país 51)
    if (phonesMatch(subPhone, clientPhone)) {
        return true;
    }

    // D) Coincidencia flexible por Nombre / Apellido / Alias
    if (subPerson) {
        if (clientName && (subPerson === clientName || clientName.includes(subPerson) || subPerson.includes(clientName))) {
            return true;
        }
        if (clientNick && (subPerson === clientNick || clientNick.includes(subPerson) || subPerson.includes(clientNick))) {
            return true;
        }

        const subTokens = subPerson.split(/\s+/).filter(t => t.length >= 3);
        const clientNameTokens = clientName.split(/\s+/).filter(t => t.length >= 3);
        const clientNickTokens = clientNick.split(/\s+/).filter(t => t.length >= 3);

        const hasTokenMatch = subTokens.some(token => clientNameTokens.includes(token) || clientNickTokens.includes(token));
        if (hasTokenMatch) {
            return true;
        }
    }
...
async function loadClientSubscriptions() {
    ...
    // Helper de filtrado y almacenamiento limpio
    const processSubscriptionsForClient = (rawDocs) => {
        ...
        (rawDocs || []).forEach(data => {
            ...
            const subId = data.id || data._docId || `${data.service}_${data.email}_${data.pin}_${Math.random()}`;
            matchedMap.set(subId, data);
        });
            ...
            if (isSpotifyService) {
                ...
                credentialsBlockHTML = `
                    <div class="bg-yellow-950/20 border border-yellow-500/30 rounded-2xl p-4 text-center space-y-2">
                        ...
                        <a href="https://wa.me/?text=Hola%20CuycitoGO,%20deseo%20consultar%20mi%20activación%20de%20${encodeURIComponent(sub.service)}" target="_blank" class="inline-flex items-center justify-center gap-1.5 bg-[#25D366] hover:bg-emerald-500 text-black font-black text-xs py-2 px-3 rounded-xl transition">
    ...
    if (expiringList.length > 0) {
        ...
        if (container) {
            container.innerHTML = expiringList.map(s => {
                ...
                const diffDays = Math.ceil((exp - now) / (1000 * 60 * 60 * 24));
                const price = getSubscriptionPrice(s);
                return `
...
function getAppProductionOrigin() {
    ...
    if (!hostname || hostname === 'localhost' || hostname === '127.0.0.1' || hostname === '::1' || window.location.protocol === 'file:') {
        // En entorno de pruebas local, predetermina el dominio oficial https://cuycitogo.online
        return 'https://cuycitogo.online';
    }
...
function renderProfileCatalog() {
    ...
    let filtered = productsList.filter(p => {
        ...
        const title = (p.title || '').toLowerCase();
        const isCombo = p.isCombo === true || cat === 'COMBOS' || cat.includes('COMBO') || title.includes('combo') || (p.comboServices && p.comboServices.length > 0);
        const isOffer = p.promo === true || p.isOffer === true || isCombo || cat === 'OFERTA' || title.includes('oferta') || title.includes('pack');
        ...
        const matchesSearch = !searchTerm || title.includes(searchTerm) || (p.description || '').toLowerCase().includes(searchTerm) || cat.includes(searchTerm) || ((p.comboServices || []).some(s => (s.service || '').toLowerCase().includes(searchTerm)));
